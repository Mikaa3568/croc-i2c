...
proc load_checkpoint {{checkpoint ""}} {
    ...
    if {$checkpoint eq "" || $checkpoint eq "latest"} {
        ...
        if {![llength $zips]} {utl::error CHK 1 "No checkpoint .zip found in $save_dir"; return}
    ...
    if {![llength $candidates]} {utl::error CHK 2 "No checkpoint found matching: $checkpoint (.zip/.odb)"; return}
    ...
    if {$ext eq ".zip"} {
        ...
        if {[file exists $sdc]} {read_sdc $sdc}
    } else {utl::error CHK 3 "Unsupported checkpoint type: $ext"}
}
